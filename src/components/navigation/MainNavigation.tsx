...
import { mainNavigationLinks } from "@/components/navigation/utils/mainNavigationLinks";
import Image from "next/image";

const MainNavigation = ({ handelClick }: { handelClick: () => void }) => {
  return (
          ...
          {mainNavigationLinks.map(({ id, link, href, description, image }) => (
            ...
            >
              <div className="flex flex-col w-full max-w-66 duration-500 scale-100 group-hover:scale-[1.04]">
                <TransitionLink handleClose={handelClick} href={href}>
                  {link}
                </TransitionLink>
